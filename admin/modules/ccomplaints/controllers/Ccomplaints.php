...
class Ccomplaints extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('mdl_ccomplaints');
    }
    ...
    function save_data()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data['complete'] = isset($_POST['complete']) ? 1 : 0;

            // Check for update or insert
            if (!empty($_POST['c_id'])) {
                $where['c_id'] = $_POST['c_id'];
                echo $this->mdl_ccomplaints->update_data($where, $data);
            } else {
                echo $this->mdl_ccomplaints->add_data($data);
            }
        } else {
    ...
    function view_data()
    {
        ...
        $return = $this->mdl_ccomplaints->view_data($where, $select)->result_array();
    ...
    function delete_data()
    {
        if (isset($_GET['c_id']) && $_GET['c_id']) {
            ...
            $where['c_id'] = $_GET['c_id'];
            echo $this->mdl_ccomplaints->delete_data($where);
        } else
            echo "Not Deleted";
    }
